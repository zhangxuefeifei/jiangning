              ...
              if (ret.exist) {
                  ...
                  if (isUpdate) {
                      ...
                      firstLoginHasZip(versionInformation[i],function(rep){
                        ...
                        if(!rep){
                          // console.log(JSON.stringify(versionInformation[i]))
                          removeFilesToLocal(versionInformation[i]);
                          deleteLocalWgt(app);
                          downLoadZipToLocal(moduleName, downloadUrl, versionInformation[i], versionInformation, isUpdate);
                        }
...
function firstLoginHasZip(singleVersionInformation,callback){ //第一次登陆有zip包的时候
  ...
  switch (true) {
      ...
      case singleVersionInformation.moduleCode == "WaterStarOne-Cloud-S9": //公共文件
          var wgtName = "public";
          break;
      case singleVersionInformation.moduleCode == "WaterStarOne-RM-S9": //决策系统
          var wgtName = "decision";
          break;
      case singleVersionInformation.moduleCode == "WaterStarOne-SNTGIS": //GIS
          var wgtName = "GIS";
          break;
      case singleVersionInformation.moduleCode == "WaterStarOne-Inspection-S9": //巡检系统
          var wgtName = "inspection";
          break;
      case singleVersionInformation.moduleCode == "WaterStarOne-CRM-S8": //客户系统
          var wgtName = "Customer";
          break;
    case singleVersionInformation.moduleCode == "WaterStarOne-RIM-S8": //报装系统
          var wgtName = "baozhuang";
          break;
  }
  ...
  fs.exist({
      ...
      if (ret.exist) {
          ...
          fs.open({
              ...
              // console.log(JSON.stringify(ret1.fd));
              var fdUrl = ret1.fd;
              if (ret1.status) {
                  fs.read({
                      fd:fdUrl,
  ...
  // console.log(JSON.stringify(req));
                      if (req.status) {
                        ...
                        var version = req.data.match(/-?([1-9]\d*(\.\d*)*|0\.[1-9]\d*)/g);
                        // console.log(version);
                        if(singleVersionInformation.newVersionNo == 0){
                          if(version == singleVersionInformation.versionNo){
                            callback(true);
                          }else{
                           callback(false);
                          }
        ...
        fs.read({
            ...
            if (ret3.status) {
              if(ret3.data == ""){
                ...
                var version = ret3.data.match(/-?([1-9]\d*(\.\d*)*|0\.[1-9]\d*)/g);
              // var version = ret3.data.substring(ret3.data.lastIndexOf(':')+1,ret3.data.length);
              if(singleVersionInformation.newVersionNo == 0){
                  updateLocalZipData(isUpdate,singleVersionInformation,versionInformation);
              }else{
                if(version == singleVersionInformation.newVersionNo){
                 updateLocalZipData(isUpdate,singleVersionInformation,versionInformation);
                }else{
...
function updateLocalZipData(isUpdate,singleVersionInformation,versionInformation){ //更新本地数据信息
  var db = api.require("db");
  if (isUpdate) {
